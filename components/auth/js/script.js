const root = ReactDOM.createRoot(document.getElementById('root'));
        ...
        function LoginSignupComponent() {
            ...
            const ErrorBanner = error ? (
                <div role="alert" style={{ width: "100%", background: "#2a1212", border: "1px solid #5c2626", color: "#ffb4b4", borderRadius: 6, padding: "0.55rem 0.7rem", fontSize: "0.8rem", marginBottom: "0.65rem", textAlign: "left" }}>
                    {error}
                </div>
            ) : null;
            ...
            const Footer = (
                <div style={{ marginTop: "0.85rem", fontSize: "0.75rem", color: "#666", lineHeight: 1.5, textAlign: "center" }}>
                    By proceeding, you agree to creating account<br />subject to our{" "}
                        ...
                        {isLogin ? (
                            <div style={{ width: "100%", maxWidth: 360, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                                ...
                                <form onSubmit={submit} style={{ width: "100%", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                                    <input style={input} type="email" placeholder="name@work-email.com" autoComplete="email" required
                                        value={email} onChange={e => setEmail(e.target.value)} />
                                    <input style={input} type="password" placeholder="Password" autoComplete="current-password" required
                                        value={password} onChange={e => setPassword(e.target.value)} />
                                    <button type="submit" disabled={busy} style={{ width: "100%", padding: "0.65rem", borderRadius: 6, border: "none", background: busy ? "#8a8a8a" : "#ededed", color: "#000", fontWeight: 500, fontSize: "0.875rem", cursor: busy ? "default" : "pointer" }}>{busy ? "Signing in…" : "Continue with Email"}</button>
                                </form>

                                <div style={{ height: 1, background: "#222", width: "100%", margin: "0.85rem 0" }} />

                                <button type="button" onClick={notImplemented} style={socialBtn}>{GoogleIcon}Continue with Google</button>
                                <button type="button" onClick={notImplemented} style={socialBtn}>{GitHubIcon}Continue with GitHub</button>
                                <button type="button" onClick={notImplemented} style={{ ...socialBtn, marginBottom: 0 }}>{AppleIcon}Continue with Apple</button>

                                <div style={{ marginTop: "1.25rem", fontSize: "0.875rem", color: "#888" }}>
                                    Don't have an account?{" "}
                                    <button type="button" onClick={() => switchMode(false)} style={{ color: "#fff", fontWeight: 500, background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit" }}>Sign Up</button>
                                </div>
                                {Footer}
                            </div>
                        ) : (
                            <div style={{ width: "100%", maxWidth: 360, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                                ...
                                {ErrorBanner}
                                <form onSubmit={submit} style={{ width: "100%", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                                    <input style={input} type="text" placeholder="Full Name" autoComplete="name" required
                                        value={name} onChange={e => setName(e.target.value)} />
                                    <input style={input} type="email" placeholder="name@work-email.com" autoComplete="email" required
                                        value={email} onChange={e => setEmail(e.target.value)} />
                                    <input style={input} type="password" placeholder="Password (at least 8 characters)" autoComplete="new-password" minLength={8} required
                                        value={password} onChange={e => setPassword(e.target.value)} />
                                    <button type="submit" disabled={busy} style={{ width: "100%", padding: "0.65rem", borderRadius: 6, border: "none", background: busy ? "#8a8a8a" : "#ededed", color: "#000", fontWeight: 500, fontSize: "0.875rem", cursor: busy ? "default" : "pointer" }}>{busy ? "Creating account…" : "Sign Up with Email"}</button>
                                </form>

                                <div style={{ height: 1, background: "#222", width: "100%", margin: "0.85rem 0" }} />

                                <button type="button" onClick={notImplemented} style={socialBtn}>{GoogleIcon}Sign up with Google</button>
                                <button type="button" onClick={notImplemented} style={socialBtn}>{GitHubIcon}Sign up with GitHub</button>
                                <button type="button" onClick={notImplemented} style={{ ...socialBtn, marginBottom: 0 }}>{AppleIcon}Sign up with Apple</button>

                                <div style={{ marginTop: "1.25rem", fontSize: "0.875rem", color: "#888" }}>
                                    Already have an account?{" "}
                                    <button type="button" onClick={() => switchMode(true)} style={{ color: "#fff", fontWeight: 500, background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit" }}>Sign In</button>
                                </div>
                                {Footer}
                            </div>
                        )}
